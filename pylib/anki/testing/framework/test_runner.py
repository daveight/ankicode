...
from anki.testing.framework.deepdiff import DeepDiff
from abc import abstractmethod, ABC
from os.path import normpath
from typing import List, Optional, Tuple

from anki.testing.framework.io_utils import non_blocking_readlines
from anki.testing.framework.test_suite_gen import START_USER_SRC_MARKER
from anki.testing.framework.types import SrcFile, TestResponse, TestSuiteExecOpts
...
def get_resource_path():
    ...
    if isMac and 'RESOURCEPATH' in os.environ:
        ...
        """ dev mode """
        result = os.path.join(pathlib.Path(__file__).parents[4], 'testing')
    return '"' + result + '"'
...
class TestRunner(ABC):
    ...
    def run(self, src_code: str, test_cases: List[str], opts: TestSuiteExecOpts, logger: ConsoleLogger):
        ...
        """
        if self.proc is not None or self.compile_proc is not None:
            raise Exception('Another test is already running ' + str(self.proc.pid))
        ...
        try:
            ...
            if compile_cmd:
                ...
                self.compile_proc = subprocess.Popen(compile_cmd,
                                                     shell=True,
            ...
            self.proc = subprocess.Popen(run_cmd,
                                         shell=True,
                                         stdin=subprocess.PIPE,
                                         stdout=subprocess.PIPE,
                                         stderr=subprocess.PIPE,
                                         text=True)
            line_iterator = non_blocking_readlines(self.proc.stdout)
            error_iterator = non_blocking_readlines(self.proc.stderr)
