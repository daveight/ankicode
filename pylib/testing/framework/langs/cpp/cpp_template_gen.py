...
class CppTemplateGenerator(SolutionTemplateGenerator):
    """
...
class Solution {
public:
\t%(result_type)s %(func_name)s(%(arg_declarations)s) {
\t\t//Add code here
\t}
};'''
    ...
    def generate_solution_template(self, ts: TestSuite) -> str:
        """
        Generates C++ source code template for the input test suite
        :param ts: input test suite
        :return: C++ source code with the solution template
        """
