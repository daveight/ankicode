## Video drivers/hardware acceleration. Please avoid translating 'OpenGL' and 'ANGLE'.

preferences-video-driver = Görüntü Sürücüsü: { $driver }
preferences-video-driver-opengl-mac = OpenGL (Mac sistemleri için önerilir)
preferences-video-driver-software-mac = Yazılım (önerilmez)
preferences-video-driver-opengl-other = OpenGL (Daha hızlı, problemlere yol açabilir)
preferences-video-driver-software-other = Yazılım (Daha yavaş)
preferences-video-driver-angle = ANGLE (OpenGL'den daha iyi çalışabilir)
