## Video drivers/hardware acceleration. Please avoid translating 'OpenGL' and 'ANGLE'.

preferences-video-driver = Відэадрайвер: { $driver }
preferences-video-driver-opengl-mac = OpenGL (рэкамендавана на прыладах Mac)
preferences-video-driver-software-mac = Праграмны (не рэкамендавана)
preferences-video-driver-opengl-other = OpenGL (хутчэйшы, можа прывесці да праблем)
preferences-video-driver-software-other = Праграмны (павольнейшы)
preferences-video-driver-angle = ANGLE (можа працаваць лепш за OpenGL)
